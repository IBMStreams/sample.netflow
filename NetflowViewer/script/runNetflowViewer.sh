...
buildDirectory=$projectDirectory/output/$composite
...
toolkitList=(
    $STREAMS_INSTALL/toolkits/com.ibm.streamsx.network
    $STREAMS_INSTALL/toolkits/com.ibm.streamsx.inetserver
    $STREAMS_INSTALL/toolkits/com.ibm.streamsx.inet
)
...
networkIF=`ifconfig | head -1 | awk -F': flags' '{print $1}'`

echo "networkInterface = $networkIF"

submitParameterList=(
    networkInterface=$networkIF
    geoDirectory=/etc/geo
    localLocationID=RCX
...
step "unzip geo files... "
unzip -o etc/geo/mergedIBMandMaxmindData.zip  -d etc/geo
...
#standalone=$buildDirectory/bin/standalone
